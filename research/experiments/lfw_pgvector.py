...
def materialize_lfw_templates(
    ...
    *,
    ...
    actions = {
        ...
        "template_256_inserted": 0,
        "template_256_skipped": 0,
    }
    ...
    with session_scope(engine) as session:
        ...
        for index, probe in enumerate(probes.itertuples(index=False), start=1):
            if compression_profile == PCA_256:
                ranked = repository.find_similar_templates_256(
    ...
    *,
    ...
    with session_scope(engine) as session:
        ...
        for index, probe in enumerate(probes.itertuples(index=False), start=1):
            ...
            pca_exact = repository.find_similar_templates_256(
                probe.retrieval_embedding,
                **scope.repository_kwargs(),
                vector_type=PCA_256,
                top_k=candidate_k,
            ...
            pca_hnsw = repository.find_similar_templates_256(
                probe.retrieval_embedding,
                **scope.repository_kwargs(),
                vector_type=PCA_256,
                top_k=candidate_k,
            ...
            certified.update(
                {
                    ...
                    "candidate_contains_pca_exact_top1": str(pca_exact_top1["identity_id"]) in hnsw_ids,
